...
find . -name "cuda_int4_matmul_ops*.so" -delete
...
# 构建扩展
echo "开始构建 Int4 MatMul CUDA 扩展..."
python3 setup_int4_only.py build_ext --inplace

if [ $? -eq 0 ]; then
    echo "构建成功！"
    echo "生成的文件:"
    find . -name "cuda_int4_matmul_ops*.so" -exec ls -la {} \;
    
    echo "运行测试..."
    python3 examples/int4_matmul_benchmark.py
